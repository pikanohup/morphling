...
const getLayersTask = new Task(async (driver, url) => {
  const logger = new Logger(process.stdout, DEFAULT_STABILIZE_NAMES)
  const { page, dp } = await driver.startURL(url)
  ...
  dp.LayerTree.enable()
  const initialLayers = (await dp.LayerTree.onceLayerTreeDidChange()).params.layers

  // TODO
  dp.Runtime.evaluate({ expression: 'addCompositedLayer()' })
  const modifiedLayers = (await dp.LayerTree.onceLayerTreeDidChange()).params.layers
  ...
  const attributesResponse = await dp.DOM.getAttributes({ nodeId: nodeResponse.result.nodeIds[0] })
  const attributes = attributesFromArray(attributesResponse.result.attributes)
